...
public class ViewAudioAnimation extends Animation {

    public static boolean NEED_PROXY = Build.VERSION.SDK_INT < 11;

    private static final WeakHashMap<View, ViewAudioAnimation> PROXIES = new WeakHashMap<View, ViewAudioAnimation>();

    public static ViewAudioAnimation wrap(View view) {
        ViewAudioAnimation proxy = PROXIES.get(view);
        Animation animation = view.getAnimation();
        if (proxy == null || proxy != animation && animation != null) {
            proxy = new ViewAudioAnimation(view);
    ...
    private ViewAudioAnimation(View view) {
        setDuration(0);
        setFillAfter(true);
