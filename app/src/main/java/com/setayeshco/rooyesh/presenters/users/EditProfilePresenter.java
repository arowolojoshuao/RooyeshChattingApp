...
public class EditProfilePresenter implements Presenter {
   ...
   //     data =activity.getIntent();
    ...
    public void EditCurrentName(String name,String std_number ,String password,int isProf, boolean forComplete) {
        mUsersContacts.editUsername(name,password,std_number,isProf).subscribe(statusResponse -> {
            if (statusResponse.isSuccess()) {
                PreferenceManager.setUserRole(getApplicationContext(), (isProf == 1 ? true : false));
                PreferenceManager.setUserPassword(getApplicationContext(), password);
                PreferenceManager.setStdNumber(getApplicationContext(), std_number);
                ...
                if (forComplete) {
                    ...
                    Log.d("CONFRIMED","confrimed Selectstatus >>>>>> "+ PreferenceManager.isConfrimed(completeRegistrationActivity));
            /*        if (!PreferenceManager.isConfrimed(completeRegistrationActivity)) {
                        Intent intent = new Intent(completeRegistrationActivity, ConfrimedActivity.class);
                    ...
                    if(!PreferenceManager.isConfrimed(completeRegistrationActivity)) {
                        ...
                        Intent intent1 = new Intent(completeRegistrationActivity, ConfrimedActivity.class);
                        intent1.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        completeRegistrationActivity.startActivity(intent1);
                        completeRegistrationActivity.finish();
                        AnimationsUtil.setSlideInAnimation(completeRegistrationActivity);
                    }else {
                        Intent intent = new Intent(completeRegistrationActivity, PasswordActivity.class);
    ...
    public void EditCurrentPassword(String password, boolean forComplete) {
        mUsersContacts.editCurrentPassword(password).subscribe(statusResponse -> {
            if (statusResponse.isSuccess()) {
                if (forComplete) {
                    AppHelper.Snackbar(completeRegistrationActivity.getBaseContext(), completeRegistrationActivity.findViewById(R.id.completeRegistrationLayout), statusResponse.getMessage(), AppConstants.MESSAGE_COLOR_SUCCESS, AppConstants.TEXT_COLOR);
                    PreferenceManager.setIsNeedInfo(completeRegistrationActivity, false);
    ...
    public void EditCurrentStdNumber(String stdNumber, boolean forComplete) {
        mUsersContacts.editCurrentStdNumber(stdNumber).subscribe(statusResponse -> {
            if (statusResponse.isSuccess()) {
                if (forComplete) {
                    AppHelper.Snackbar(completeRegistrationActivity.getBaseContext(), completeRegistrationActivity.findViewById(R.id.completeRegistrationLayout), statusResponse.getMessage(), AppConstants.MESSAGE_COLOR_SUCCESS, AppConstants.TEXT_COLOR);
                    PreferenceManager.setIsNeedInfo(completeRegistrationActivity, false);
                    ...
                    Intent intent = new Intent(completeRegistrationActivity, MainActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    PreferenceManager.setIsLogin(completeRegistrationActivity.getBaseContext(),true);
                    completeRegistrationActivity.startActivity(intent);
                    completeRegistrationActivity.finish();
                    AnimationsUtil.setSlideInAnimation(completeRegistrationActivity);
                } else {
